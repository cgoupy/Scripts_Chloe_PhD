...
TH1D* Cut_Histo(TH1D* h_qdc, double QDC_loweredge, double QDC_upperedge) {
    ...
    TH1D* h_qdc_cut = new TH1D(Name,
                               ...
                               h_qdc->GetBinLowEdge(h_qdc->FindBin(QDC_upperedge))
                               );
...
// --------------- Fit function
ROOT::Fit::FitResult Fit_Data_Simu(std::vector<Double_t> NumEvents, TVectorT<Double_t> E_bins, TVectorT<Double_t> Data_vect, std::vector<TVectorT<Double_t>> Simu_vect, std::vector<TMatrixT<Double_t>> V_Simu, TMatrixT<Double_t> V_Data, std::vector<TString> Names, std::vector<Double_t> ini_flux, std::vector<int> fixindex) {
...
//    }
    ...
    fitter_Fit.Config().ParSettings(1).SetLimits(0.1, 10.);
    fitter_Fit.Config().ParSettings(2).SetLimits(0.1, 10.);
    fitter_Fit.Config().ParSettings(3).SetLimits(0.1, 10.);
    fitter_Fit.Config().ParSettings(4).SetLimits(0.0001, 1.);
    fitter_Fit.Config().ParSettings(5).SetLimits(0.0001, 0.06);
    fitter_Fit.Config().ParSettings(6).SetLimits(0.0, 1.);
    
    for (int k=0; k<N_simu+1; k++){
        if (fixindex[k]==1) fitter_Fit.Config().ParSettings(k+1).Fix();
    }
...
unsigned long long SetNumEvents(TString FileName, bool Bugged_mu=false){
    ...
    if (Bugged_mu){
        ...
        histoPrim = (TH1D*) file->Get("Primary_gamma_spectrum");
        NumEvents=histoPrim->GetEntries();
    }
...
// ---------------------------
ROOT::Fit::FitResult Fit_OnRange(vector<Double_t> NumEvents, TH1D* histo_data, TH2D* Var_data, std::vector<TH1D*> histo_simu, double Emin, double Emax, std::vector<Double_t> ini_flux, std::vector<int> fixindex){
    ...
    TVectorT<Double_t> E_bins(New_Nbins) ; std::vector<TVectorT<Double_t>> Simu_vect={FormatVector, FormatVector, FormatVector, FormatVector, FormatVector, FormatVector};
    TVectorT<Double_t> Data_vect(New_Nbins);
    TMatrixT<Double_t> V_Data(New_Nbins, New_Nbins);
    std::vector<TMatrixT<Double_t>> V_Simu = {FormatMatrix, FormatMatrix, FormatMatrix, FormatMatrix, FormatMatrix, FormatMatrix};
    ...
    std::vector<TString> Names = {"U238", "K40", "Th232", "Mu", "Neutr", "Rn"};
...
void Plot_and_Save(TString fileData_name, TString file_simu_U238, TString file_simu_K40, TString file_simu_Th232, TString file_simu_mu, TString file_simu_neutr, TString file_simu_Rn, TString Name_Fileout, Double_t Emin, Double_t Emax, ROOT::Fit::FitResult fit){
    ...
    double Flux_mu = fit.Parameter(4);
    double Flux_mu_err = fit.ParError(4);
    
    double Flux_neutr = fit.Parameter(5);
    double Flux_neutr_err = fit.ParError(5);
    
    double Flux_Rn = fit.Parameter(6);
    double Flux_Rn_err = fit.ParError(6);
    
    TMatrixD Cov_Matrix(7,7);
    ...
    histo_Th232->SetName("histo_Th232");
    
    //Simu Mu
    TFile *filemu = new TFile(file_simu_mu);
    TH1D* histo_mu = (TH1D*) filemu->Get("Normalized_Evis_gamma_spectrum_dru");
    histo_mu->SetTitle("histo_mu");
    histo_mu->SetName("histo_mu");
    
    //Simu Neutr
    TFile *fileneutr = new TFile(file_simu_neutr);
    TH1D* histo_neutr = (TH1D*) fileneutr->Get("Normalized_Evis_gamma_spectrum_dru");
    histo_neutr->SetTitle("histo_neutr");
    histo_neutr->SetName("histo_neutr");
    
    //Simu Rn
    TFile *fileRn = new TFile(file_simu_Rn);
    TH1D* histo_Rn = (TH1D*) fileRn->Get("Normalized_Evis_gamma_spectrum_dru");
    histo_Rn->SetTitle("histo_Rn");
    histo_Rn->SetName("histo_Rn");
    
    TH1D* Sum= (TH1D*) histo_U238->Clone();
    Sum->Add(Sum, histo_K40);
    Sum->Add(Sum, histo_Th232);
    Sum->Add(Sum, histo_mu);
    Sum->Add(Sum, histo_neutr);
    Sum->Add(Sum, histo_Rn);
    Sum->SetTitle("Sum_simu");
    
    //Set the correct errors on Sum:
    TVectorD Fluxes(7);
    TVectorD Histo_errors(7);
    TVectorD Cov_mat_Fluxes(7);
    Double_t Error_sum;
    ...
    for (int i=0; i<Sum->GetNbinsX(); i++){
        ...
        Fluxes(4)=1./Flux_mu*histo_mu->GetBinContent(i+1);
        Fluxes(5)=1./Flux_neutr*histo_neutr->GetBinContent(i+1);
        Fluxes(6)=1./Flux_Rn*histo_Rn->GetBinContent(i+1);
        
        Histo_errors(1)=histo_U238->GetBinError(i+1);
        Histo_errors(2)=histo_K40->GetBinError(i+1);
        Histo_errors(3)=histo_Th232->GetBinError(i+1);
        Histo_errors(4)=histo_mu->GetBinError(i+1);
        Histo_errors(5)=histo_neutr->GetBinError(i+1);
        Histo_errors(6)=histo_Rn->GetBinError(i+1);
    ...
    histo_mu->Rebin(REBIN);
    histo_neutr->Rebin(REBIN);
    histo_Rn->Rebin(REBIN);
    Sum->Rebin(REBIN);
    ...
    histo_mu->Scale(1./REBIN);
    histo_neutr->Scale(1./REBIN);
    histo_Rn->Scale(1./REBIN);
    Sum->Scale(1./REBIN);
    ...
    histo_mu->Write("Simu_mu");
    histo_neutr->Write("Simu_neutr");
    histo_Rn->Write("Simu_Rn");
    Sum->Write("Simu_Sum");
    ...
    histo_mu=GoodFormat(histo_mu, "Fitted_histo_mu", New_Nbins, Emin, Emax);
    histo_neutr=GoodFormat(histo_neutr, "Fitted_histo_neutr", New_Nbins, Emin, Emax);
    histo_Rn=GoodFormat(histo_Rn, "Fitted_histo_Rn", New_Nbins, Emin, Emax);
    ...
    histo_mu->Write("Simu_mu_fit_Range");
    histo_neutr->Write("Simu_neutr_fit_Range");
    histo_Rn->Write("Simu_Rn_fit_Range");
...
void Fit(TString fileData_name, TString file_simu_U238, TString file_simu_K40, TString file_simu_Th232, TString file_simu_mu, TString file_simu_neutr, TString file_simu_Rn){
    ...
    Double_t Num_mu =SetNumEvents(file_simu_mu, true);
    cout<<"Num events Mu = "<<Num_mu<<endl;
    Double_t Num_neutr =SetNumEvents(file_simu_neutr);
    cout<<"Num events neutrons = "<<Num_neutr<<endl;
    Double_t Num_Rn =SetNumEvents(file_simu_Rn);
    cout<<"Num events Rn = "<<Num_Rn<<endl;
    vector<Double_t> NumEvents = {Num_U238, Num_K40, Num_Th232, Num_mu, Num_neutr, Num_Rn};
    ...
    //Muons
    TFile *filein_mu = new TFile(file_simu_mu);
    TH1D*  histo_simu_mu = (TH1D*) filein_mu->Get("Evis_gamma_spectrum");
    histo_simu_mu->Rebin(REBIN);
    
    //Neutrons
    TFile *filein_neutr = new TFile(file_simu_neutr);
    TH1D*  histo_simu_neutr = (TH1D*) filein_neutr->Get("Evis_gamma_spectrum");
    histo_simu_neutr->Rebin(REBIN);

    //Neutrons
    TFile *filein_Rn = new TFile(file_simu_Rn);
    TH1D*  histo_simu_Rn = (TH1D*) filein_Rn->Get("Evis_gamma_spectrum");
    histo_simu_Rn->Rebin(REBIN);
    
    std::vector<TH1D*> histos_simu = {histo_simu_U238, histo_simu_K40, histo_simu_Th232, histo_simu_mu, histo_simu_neutr, histo_simu_Rn};
    
    //Fit
    Double_t Emin=500;
    Double_t Emax=2700.;
    std::vector<Double_t> ini_flux = {1.987, 0.912, 0.038, 0.019*0.7, 0.0134/5., 8.9347797e-05};
    std::vector<int> fixindex={0,0,0,1,1,1};
    ROOT::Fit::FitResult fit = Fit_OnRange(NumEvents, histo_data, V_data, histos_simu, Emin, Emax, ini_flux, fixindex);

    int N_simu=fit.Parameter(0);
    cout<<"N_simu= "<<N_simu<<endl;
    double Flux_U238 = fit.Parameter(1);
    double Flux_U238_err = fit.ParError(1);
    ...
    TMatrixD Cov_Matrix(N_simu+1,N_simu+1);
    fit.GetCovarianceMatrix(Cov_Matrix);
    TMatrixD Corr_Matrix(N_simu+1,N_simu+1);
    ...
    cout << " flux Muons = " << Flux_mu << " +- " << Flux_mu_err << " ev/cm2/s"<<endl;
    cout << " flux Neutrons = " << Flux_neutr << " +- " << Flux_neutr_err << " ev/cm2/s"<<endl;
    cout << " flux Radon = " << Flux_Rn << " +- " << Flux_Rn_err << " ev/cm2/s"<<endl;
    
    //Calculate total flux and propagate uncertainties:
    const Double_t flux_val[7] = {0, Flux_U238, Flux_K40, Flux_Th232, Flux_mu, Flux_neutr, Flux_Rn};
    TVectorD Fluxes_values(N_simu+1, flux_val);
//    Fluxes_values.Print();
...
//    Corr_Matrix.Print();
    cout << "\n total flux gammas = " << Flux_U238 + Flux_K40 + Flux_Th232<< " +- " << Error_sum << " ev/cm2/s"<<endl;
    cout << "**********************************************************************"<<endl;

    Normalize_Simu(file_simu_U238, Flux_U238);
    Normalize_Simu(file_simu_Th232, Flux_Th232);
    Normalize_Simu(file_simu_K40, Flux_K40);
    Normalize_Simu(file_simu_mu, Flux_mu);
    Normalize_Simu(file_simu_neutr, Flux_neutr);
    Normalize_Simu(file_simu_Rn, Flux_Rn);
    ...
    TPMERegexp("Conv_Analyzer").Substitute(file_simu_mu, Form("Normalized_Flux_%0.3f_Conv_Analyzer", Flux_mu));
    TPMERegexp("Conv_Analyzer").Substitute(file_simu_neutr, Form("Normalized_Flux_%0.3f_Conv_Analyzer", Flux_neutr));
    TPMERegexp("Conv_Analyzer").Substitute(file_simu_Rn, Form("Normalized_Flux_%0.3f_Conv_Analyzer", Flux_Rn));
    
    TString Out_name_file = fileData_name;
    TPMERegexp("(\\.*\\/*\\w+\\/)*").Substitute(Out_name_file, "");
    TPMERegexp(".root").Substitute(Out_name_file, "");
    Out_name_file=Form("Fit_%s_U238_%0.3f_K40_%0.3f_Th232_%0.3f_Mu_%0.3f_Neutr_%0.3f_Rn_%0.3f.root", Out_name_file.Data(), Flux_U238, Flux_K40, Flux_Th232, Flux_mu, Flux_neutr, Flux_Rn);
    
    Plot_and_Save(fileData_name, file_simu_U238,file_simu_K40, file_simu_Th232, file_simu_mu, file_simu_neutr, file_simu_Rn, Out_name_file, Emin, Emax, fit);
