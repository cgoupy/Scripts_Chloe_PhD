...
//
        for (int jp1 = 0; jp1 < ip_COV1; jp1++) {
	  if ((T_NTD[ip] - T_COV1[jp1] - DTacc/2.) >= -2. && (T_NTD[ip] - T_COV1[jp1] - DTacc/2.) < 3.) { 
            for (int jp2 = 0; jp2 < ip_COV2; jp2++) {
              if ((T_NTD[ip] - T_COV2[jp2] - DTacc/2.) >= -2. && (T_NTD[ip] - T_COV2[jp2] - DTacc/2.) < 3.) {
                Flag_noCOV1andCOV2 = true;
              }
            }
            for (int jp2 = 0; jp2 < ip_COV2; jp2++) {
              if ((T_NTD[ip] - T_COV2[jp2] + DTacc/2.) >= -2. && (T_NTD[ip] + T_COV2[jp2] - DTacc/2.) < 3.) {
...
//
	  ...
	  if ((T_NTD[ip] - T_COV1[jp1] + DTacc/2.) >= -2. && (T_NTD[ip] - T_COV1[jp1] + DTacc/2.) < 3.) { 
        ...
        if (Flag_noCOV1andCOV2 == true) h1_NTD_5_MeV->Fill(P_NTD[ip]/CalNTD/1000.);

//
        for (int jp1 = 0; jp1 < ip_COV1; jp1++) {
	  if ((T_NTD[ip] - T_COV1[jp1]) >= -2. && (T_NTD[ip] - T_COV1[jp1]) < 3.) { 
            for (int jp2 = 0; jp2 < ip_COV2; jp2++) {
              if ((T_NTD[ip] - T_COV2[jp2]) >= -2. && (T_NTD[ip] - T_COV2[jp2]) < 3.) {
